...
app.get('/account/:account', async function (req, res) {
  ...
  let nfts;
  try {
    nfts = await util.get_nfts_for_account(account, {detect_change_send: req.query.detect_change_send === "1", offset: Number(req.query.offset), supporter: supporter, recursive: false});
  } catch (e) {
    console.log(e);
